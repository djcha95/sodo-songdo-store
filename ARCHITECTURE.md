...
-   **Firebase 서버에서 실행되는 백엔드 로직을 담고 있습니다. 카카오 로그인 토큰 검증, 포인트 변동 시 자동 알림 생성 등 핵심 서버 로직과 더불어, 아래의 자동화 기능이 포함되어 있습니다.**
    ...
    - **`sendPickupReminders`**: 매일 아침 9시에 실행되어, 그날 픽업이 시작되는 고객에게 상품의 특성(당일 마감, 일반)에 따라 개인화된 픽업 안내 알림톡을 자동으로 발송합니다.
    - **`sendPrepaymentReminders`**: 매일 저녁 7시에 실행되어, 당일 마감 상품을 아직 수령하지 않은 고객에게 '선입금' 전환을 안내하는 최종 리마인더 알림톡을 발송하여 노쇼를 방지합니다.
