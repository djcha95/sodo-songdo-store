...
6.  **✅ [신규] 대기자 관리 및 자동 예약 전환**
    * **동작:** 관리자가 '통합 판매 관리' 페이지에서 대기자가 있는 상품의 **대기자 수**를 클릭하면, 해당 상품의 대기자 명단을 신청 순서대로 보여주는 모달 창이 열립니다. 관리자는 이 모달에서 추가할 재고 수량을 입력하고 **'재고 추가 및 자동 전환'** 버튼을 클릭합니다. 시스템은 추가된 재고만큼 대기 순번이 빠른 사용자부터 순서대로 '예약 확정' 상태로 자동 전환하고, 처리 결과를 즉시 피드백합니다.
