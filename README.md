...
-   **주문 후 처리 로직 개선**: 주문이 완료된 '예약' 상품만 장바구니에서 깔끔하게 제거하는 `removeReservedItems` 함수를 추가하여 효율성을 높였습니다.
...
-   **통합 상태 렌더링**: `useCart` 훅에서 가져온 단일 `cartItems` 배열을, 상품의 `status`에 따라 '예약 상품'과 '대기 상품'으로 실시간으로 필터링하여 각 섹션에 표시합니다.
-   **일관된 사용자 경험**: '대기' 상태의 상품도 '예약' 상품과 동일하게 수량 변경, 삭제 등의 상호작용이 가능하도록 구현되었습니다.
