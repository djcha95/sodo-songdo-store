...
`orderService.ts`: 결제 완료된 '주문' 내역을 관리합니다.
...
5.  **대기(Waitlist) 기능**
    * **동작:** 한정 수량 상품이 모두 판매되어 품절되었을 경우, 사용자는 '대기' 버튼을 통해 재입고 알림을 신청할 수 있습니다.
    * 관리자는 대기 명단을 확인하고, 추가 물량을 확보했을 때 해당 사용자들에게 알림을 보내거나 자동으로 예약을 생성해 줄 수 있습니다. (후속 기능)
    * **핵심 파일:** `ProductCard.tsx`, `productService.ts`, `types.ts`
