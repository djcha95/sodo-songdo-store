...
7.  **지능형 예약 내역 관리**
    ...
    * **두 가지 보기 모드 제공**: '예약 건별 보기'는 사용자의 개별 주문 내역을 주문 단위로 그룹화하여 보여주며, '픽업일 순 보기'는 여러 주문에 흩어진 동일 상품을 픽업일에 따라 자동 합산하여 "다음에 무엇을 총 몇 개 찾아가야 하는지"를 명확히 알려줍니다.
...
### 지능형 예약 내역 페이지(`OrderHistoryPage.tsx`) 구현

-   **문제점**: 기존의 단순 주문 목록은 고객이 "언제 무엇을 찾아가야 하는지" 직관적으로 파악하기 어려웠고, 예약 취소 정책이 명확하지 않았습니다.
-   **해결**:
    -   **두 가지 보기 모드 도입**: '주문일 순 보기'와 '픽업일 순 보기' 모드를 제공하여 사용자가 원하는 기준으로 예약 내역을 조회할 수 있도록 개선했습니다.
    -   **데이터 집계 로직 구현**: '픽업일 순 보기'를 위해, 여러 주문에 흩어져 있는 상품들을 픽업 날짜별로 그룹화하고 수량을 합산하는 로직을 `useMemo`를 활용하여 효율적으로 구현했습니다.
    -   **동적인 취소 정책 적용**: 픽업까지 남은 시간 등을 고려하여 '일반 취소'와 '신중 취소' 버튼을 동적으로 표시하고, `toast`를 통해 사용자에게 명확한 경고와 확인 절차를 제공하도록 UI를 설계했습니다.
    -   **애니메이션을 통한 UX 향상**: `framer-motion` 라이브러리를 사용해 '픽업일 순 보기'의 아코디언 메뉴가 부드럽게 열리고 닫히도록 하여 시각적인 만족도를 높였습니다.
