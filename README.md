...
│   ├── firebase/        # Firebase 초기 설정 및 기능별 서비스 모음 (productService 등)
...
### `src/firebase/index.ts` (구 `src/firebase.ts`)

-   Firebase와의 모든 통신을 담당하는 중앙 허브 역할을 합니다. 기존에는 모든 함수가 이 파일에 있었으나, 리팩토링을 통해 productService, orderService 등 기능별로 분리된 파일들을 통합하여 내보내는(export) 역할로 변경되었습니다. 다른 파일에서는 여전히 이 파일을 통해 import하여 코드의 일관성을 유지합니다.
...
-   고객용 앱의 **메인 페이지**에 해당하는, 가장 핵심적인 페이지 컴포넌트입니다.
-   Firebase로부터 데이터를 불러와 `공동구매 진행 중`, `마감 임박! 추가 예약`, `지난 공동구매` 등의 섹션으로 동적으로 분류하여 표시합니다.
-   `useMemo`를 활용하여 상품 데이터가 변경될 때만 섹션별 상품 목록을 재계산하여 성능을 최적화합니다.
-   페이지의 각 섹션은 재사용 가능한 **`<ProductSection />` 컴포넌트**를 통해 렌더링됩니다.
-   상품 카드의 시각적 표현과 상호작용 로직을 담당하는 **`<MemoizedProductCard />`**가 이 파일에 정의되어 있습니다.
-   기존에 이 파일에 포함되어 있던 상품 섹션과 카드 UI 로직이 각각 `ProductSection.tsx`와 `ProductCard.tsx`로 분리되어, 이 파일은 데이터 관리와 페이지 레이아웃에 더 집중하게 되었습니다.
...
-   **정교화된 구매 로직**: 상품의 상태('판매 중', '추가 예약', '판매 종료', '품절' 등)에 따라 '장바구니 담기' 또는 '앵콜 요청' 버튼을 동적으로 정확하게 표시합니다.
...
-   URL 진입 방식에 따라 두 가지 모드로 동작하는 동적인 페이지입니다.
    -   **신규 상품 등록 모드:** 완전히 새로운 대표 상품의 고유 정보(이름, 설명, 이미지 등)와 **첫 번째 판매 회차** 정보를 함께 등록합니다.
    -   **새 회차 추가 모드:** 기존 대표 상품에 대한 **다음 판매 회차**의 정보(회차명, 가격, 재고, 기간 등)만을 추가합니다. 이 모드에서는 대표 상품 정보는 수정할 수 없습니다.
...
2.  **상품 목록 보기 (메인 페이지)**
    * **동작:** 로그인한 사용자가 가장 처음 보게 되는 메인 페이지입니다. 상품 데이터는 상태별(`진행 중`, `마감 임박`, `지난 상품`)로 명확히 구분된 섹션에 표시됩니다. 개선된 상품 카드는 사용자에게 **한정 재고, 픽업일, 보관 타입 등 핵심 정보를 한눈에 제공**하며, **단일/복합 상품에 따라 최적화된 구매 버튼**을 통해 즉각적인 상호작용을 유도합니다.
    * **핵심 파일:** `ProductListPage.tsx`, `ProductSection.tsx`, `firebase.ts`

3.  **상품 상세 정보 보기**
    * **동작:** 메인 페이지에서 특정 상품을 클릭했을 때, **기존 페이지의 스크롤 위치는 그대로 유지된 채** 해당 상품의 상세 정보가 담긴 모달(modal) 창이 부드럽게 나타납니다. 장바구니 담기, 수량 조절 등의 상호작용 시 `toast` 알림으로 즉각적인 피드백을 제공합니다. 상품의 판매 상태에 따라 '장바구니 담기' 또는 '앵콜 요청' 버튼이 동적으로 표시됩니다.
    * **핵심 파일:** `ProductDetailPage.tsx`, `CustomerLayout.tsx`, `ProductListPage.tsx`

4.  **장바구니 관리**
    * **동작:** 사용자가 상품 상세페이지에서 원하는 옵션과 수량을 선택하여 장바구니에 담고, 장바구니 페이지에서 최종적으로 하나의 주문(예약)으로 생성하는 과정입니다. 장바구니 추가, 수량 변경 등 모든 액션에 대해 `toast` 알림을 활용하여 사용자 경험을 개선합니다.
    * **핵심 파일:** `ProductDetailPage.tsx`, `CartPage.tsx`, `CartContext.tsx`, `firebase.ts`
...
6.  **주문 및 내역 확인**
    * **동작:** 사용자가 마이페이지를 통해 `orders` 컬렉션에 기록된 자신의 이전 주문(예약) 내역들을 확인하는 과정입니다. 주문 상태 변경 등 중요한 알림이 필요한 경우 `toast`를 사용할 수 있습니다.
...
5.  **재고 및 현장 판매 관리**
    * **동작:** 관리자가 상품의 재고를 직접 수정하고, 오프라인 현장 판매 가능 여부를 설정하는 기능입니다. 재고 변경 및 현장 판매 설정 시 `toast` 알림으로 관리자에게 피드백을 제공합니다.
    * **핵심 파일:** `ProductListPageAdmin.tsx`, `firebase.ts`
...
### **[추가]** 최근 주요 리팩토링 및 구조 개선 (2025년 7월)

#### 1. 코드 구조 개선: 역할과 책임의 명확한 분리 🧩

-   **Firebase 통신 분리**: 기존의 거대한 `firebase.ts` 파일을 기능별로 명확하게 분리했습니다. 이제 `productService.ts`는 상품 관련 데이터만, `orderService.ts`는 주문 관련 데이터만 책임집니다. 이 모든 서비스는 `firebase/index.ts`를 통해 통합 관리되어 코드의 예측 가능성과 테스트 용이성이 크게 향상되었습니다.
-   **UI 컴포넌트 분리**: `ProductListPage.tsx`에 복잡하게 얽혀있던 UI 로직을 `ProductCard.tsx`와 `ProductSection.tsx`라는 독립적인 컴포넌트로 분리했습니다. 이로써 `ProductListPage`는 데이터 흐름과 페이지 레이아웃에만 집중하는 '컨테이너' 역할을, 각 컴포넌트는 UI 표현에만 집중하는 '프레젠테이션' 역할을 수행하게 되어 코드 관리가 훨씬 수월해졌습니다.

#### 2. 유지보수성 및 가독성 향상: "누가 봐도 이해하기 쉬운 코드" 📖

-   **절대 경로 도입**: `tsconfig.app.json`에 `@` 경로 별칭을 설정하여, `../../components`와 같은 복잡하고 오류 발생 쉬운 상대 경로 대신 `@/components`처럼 직관적이고 안정적인 절대 경로를 사용하도록 통일했습니다.
-   **컴포넌트별 스타일 관리**: `ProductSection.css` 파일을 생성하여 `ProductSection` 컴포넌트만의 스타일을 독립적으로 관리하도록 개선했습니다. 이를 통해 컴포넌트의 응집도를 높이고 다른 컴포넌트와의 스타일 충돌 가능성을 원천적으로 차단했습니다.

#### 3. 안정성 강화: Zod를 통한 데이터 유효성 검사 🛡️

-   **Zod 스키마 도입**: `types.ts` 파일에 `zod` 스키마를 정의하고, `productService.ts`에서 Firestore 데이터를 불러올 때 이 스키마를 통해 데이터의 타입과 형식을 런타임에 검증하는 로직을 추가했습니다.
-   **기대 효과**: 만약 데이터베이스에 잘못된 데이터(예: 상품 가격이 숫자가 아닌 문자열)가 저장되어 있더라도, 더 이상 앱이 비정상적으로 중단되지 않습니다. 대신, 개발자 콘솔에 어떤 데이터가 문제인지 명확한 오류 로그가 남겨 버그의 원인을 즉시 파악하고 대응할 수 있게 되었습니다.

#### 4. 개발 환경 문제 해결

-   `EBUSY: resource busy or locked` 오류, `auth/invalid-api-key` 오류 등 로컬 개발 환경에서 발생하던 여러 문제를 해결하여 안정적인 개발 및 테스트 환경을 구축했습니다.
