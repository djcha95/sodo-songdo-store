...
// ... 기존 sendPickupReminders, sendPrepaymentReminders 함수 코드 ...
...
// ✅ [신규] 매일 오후 1시, 예약 확정 일괄 안내 알림톡 발송 함수
export const sendOrderConfirmationReminders = onSchedule(
  {
    schedule: "every day 13:00", // 매일 오후 1시에 실행
    timeZone: "Asia/Seoul",
    region: "asia-northeast3",
  },
  async (context: ScheduledEvent) => {
    logger.info("오후 1시: 예약 확정 일괄 안내 알림톡 발송 작업을 시작합니다.");
    try {
        const db = getFirestore();
        const now = Timestamp.now();
        const twentyFourHoursAgo = Timestamp.fromMillis(now.toMillis() - (24 * 60 * 60 * 1000));

        // 지난 24시간 동안 생성된 예약/선결제 주문 조회
        const ordersSnapshot = await db.collection("orders")
        ...
        if (ordersSnapshot.empty) {
            logger.info("지난 24시간 동안 생성된 예약 확정 대상 주문이 없습니다. 작업을 종료합니다.");
            return;
        }
        ...
        ordersSnapshot.forEach(doc => {
            ...
            // "즉시 픽업" 건은 이미 알림이 갔으므로 제외
            const pickupStartDate = normalizeToDate(order.pickupDate);
            const createdAtDate = normalizeToDate(order.createdAt);
            if (!pickupStartDate || !createdAtDate) return;
            ...
            if (pickupStartDateOnly <= todayStart) {
                logger.info(`주문(${order.id})은 즉시 픽업 건이므로 이번 일괄 발송에서 제외합니다.`);
                return;
            }

            const existingOrders = ordersByUser.get(order.userId) || [];
            ordersByUser.set(order.userId, [...existingOrders, order]);
        });

        if (ordersByUser.size === 0) {
            logger.info("모든 주문이 즉시 픽업 건으로 확인되어, 발송할 예약 확정 알림이 없습니다.");
            return;
        }
        ...
        for (const [userId, userOrders] of ordersByUser.entries()) {
            ...
            // 가장 빠른 픽업 시작일 계산
            const earliestPickupDate = new Date(Math.min(
                ...userOrders.map(order => normalizeToDate(order.pickupDate)!.getTime())
            ));
            ...
            const templateVariables: { [key: string]: string } = {
                고객명: userData.displayName,
                상품목록: productList,
                픽업시작일: formatDate(earliestPickupDate),
            };
            
            await sendAlimtalk(userData.phone, templateCode, templateVariables);
        }

        logger.info(`${ordersByUser.size}명의 사용자에게 예약 확정 일괄 안내 알림톡 발송을 완료했습니다.`);
    } catch (error) {
        logger.error("오후 1시 예약 확정 일괄 안내 알림톡 발송 중 오류 발생:", error);
    }
  });