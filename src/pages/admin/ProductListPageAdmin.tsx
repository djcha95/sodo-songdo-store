...
// ✅ [성능 최적화] 여러 개의 useState를 하나의 state 객체로 통합했습니다.
// fetchData 함수에서 모든 비동기 요청이 완료된 후 state를 한 번만 업데이트하여,
// 복잡한 useMemo 연산(enrichedRounds)이 불필요하게 여러 번 실행되는 것을 방지하고 성능을 최적화합니다.
...
const translateStatus = (status: SalesRoundStatus): string => {
    const statusMap: Record<SalesRoundStatus, string> = { selling: '판매중', scheduled: '판매예정', ended: '판매종료', sold_out: '품절', draft: '임시저장' };
...
const ProductAdminRow: React.FC<ProductAdminRowProps> = ({ item, index, isExpanded, isSelected, editingStockId, stockInputs, onToggleExpansion, onSelectionChange, onStockEditStart, onStockEditSave, onSetStockInputs, onOpenWaitlistModal }) => {
    ...
    if (!item.enrichedVariantGroups || item.enrichedVariantGroups.length === 0) {
        return (<tr className="master-row error-row"><td><input type="checkbox" checked={isSelected} onChange={(e) => onSelectionChange(item.uniqueId, e.target.checked)} /></td><td>{index + 1}</td><td colSpan={10}>데이터 오류: 이 회차에 옵션 그룹이 없습니다. (ID: {item.uniqueId})</td><td><div className="action-buttons-wrapper"><button onClick={() => navigate(`/admin/products/edit/${item.productId}/${item.round.roundId}`)} className="admin-action-button"><Edit size={16}/></button></div></td></tr>);
    }
    ...
    if (!isExpandable) {
        ...
        return (
          <tr className={rowClass}>
            ...
            <td style={{textAlign: 'right'}}>{vg.items[0]?.price.toLocaleString() ?? '–'} 원</td><td>{formatDate(getEarliestExpirationDateForGroup(vg))}</td>
    ...
    return (
      <React.Fragment>
        <tr className="master-row expandable">
          ...
          <td style={{textAlign: 'center', color: 'var(--text-color-light)'}}>–</td><td>{formatDate(earliestOverallExpiration)}</td><td style={{textAlign: 'center', color: 'var(--text-color-light)'}}>–</td><td style={{textAlign: 'center', color: 'var(--text-color-light)'}}>–</td><td style={{textAlign: 'center', color: 'var(--text-color-light)'}}>–</td>
          <td><div className="action-buttons-wrapper"><button onClick={() => navigate(`/admin/products/edit/${item.productId}/${item.round.roundId}`)} className="admin-action-button" title="이 판매 회차 정보를 수정합니다."><Edit size={16}/></button><button onClick={handleAddNewRound} className="admin-action-button" title="이 상품의 새 판매 회차를 추가합니다."><Plus size={16} /></button></div></td>
        </tr>
        {isExpanded && item.enrichedVariantGroups.map((subVg, vgIndex) => {
          ...
          return (
              <tr key={subVgUniqueId} className={subRowClasses}>
                  ...
                  <td style={{textAlign: 'right'}}>{subVg.items[0]?.price.toLocaleString() ?? '–'} 원</td><td>{formatDate(getEarliestExpirationDateForGroup(subVg))}</td>
...
// 📌 메인 컴포넌트
// =================================================================
// ✅ [성능 최적화] 여러 state를 하나의 객체로 통합하여 불필요한 리렌더링을 방지합니다.
...
const ProductListPageAdmin: React.FC = () => {
  ...
  // ✅ [성능 최적화] 데이터 관련 state를 하나로 통합합니다.
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
        ...
        // ✅ [성능 최적화] 모든 데이터를 한 번에 업데이트하여 리렌더링을 최소화합니다.
        setPageData({
  ...
  const enrichedRounds = useMemo<EnrichedRoundItem[]>(() => {
    ...
    (pageData.allProducts || []).forEach(p => {
        (p.salesHistory || []).forEach(r => {
            ...
            flatRounds.push({ productId: p.id, productName: p.groupName, productImage: p.imageUrls?.[0] || '/placeholder.svg', category: p.category || '미지정', storageType: p.storageType, round: r, uniqueId: `${p.id}-${r.roundId}`, enrichedVariantGroups });
        });
    });
  ...
  // ... (나머지 핸들러 함수들은 기존 로직과 거의 동일하게 유지)
  ...
  const handleStockEditSave = async (vgUniqueId: string) => {
    ...
    await toast.promise(promise, { loading: "재고 정보 업데이트 중...", success: "재고가 성공적으로 업데이트되었습니다!", error: "업데이트 중 오류가 발생했습니다.", });
    fetchData();
  };
           ...
           <>
            <div className="product-list-controls-v2">
                ...
                <div className="filter-sort-wrapper">
                    <div className="control-group"><Filter size={16} /><select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="control-select"><option value="all">모든 카테고리</option>{pageData.categories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}</select><select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as any)} className="control-select"><option value="all">모든 상태</option><option value="selling">판매중</option><option value="scheduled">판매예정</option><option value="sold_out">품절</option><option value="ended">판매종료</option></select></div>
                </div>
            ...
            <div className="admin-product-table-container">
              <table className="admin-product-table">
                <thead>
                  <tr>
                    <th><input type="checkbox" checked={isAllSelected} onChange={handleSelectAll} title="전체 선택/해제"/></th><th>No.</th>
                    ...
                    <th>보관</th><th className="sortable-header" onClick={() => handleSortChange('productName')}>상품명 / 회차명 {sortConfig.key === 'productName' && (sortConfig.direction === 'asc' ? '▲' : '▼')}</th>
                    <th>상태</th><th>가격</th><th className="sortable-header" onClick={() => handleSortChange('expirationDate')}>유통기한 {sortConfig.key === 'expirationDate' && (sortConfig.direction === 'asc' ? '▲' : '▼')}</th>
                    <th title="예약된 수량 / 전체 대기자 수">예약/대기</th><th title="픽업 완료된 수량">픽업</th><th title="클릭하여 재고 수정">재고</th><th>관리</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedRounds.length > 0 ? ( paginatedRounds.map((item, index) => (<ProductAdminRow key={item.uniqueId} item={item} index={(currentPage - 1) * itemsPerPage + index} isExpanded={expandedRoundIds.has(item.uniqueId)} isSelected={selectedItems.has(item.uniqueId)} editingStockId={editingStockId} stockInputs={stockInputs} onToggleExpansion={toggleRowExpansion} onSelectionChange={handleSelectionChange} onStockEditStart={handleStockEditStart} onStockEditSave={handleStockEditSave} onSetStockInputs={setStockInputs} onOpenWaitlistModal={handleOpenWaitlistModal}/>)) ) : (<tr><td colSpan={13} className="no-products-cell">표시할 판매 회차가 없습니다.</td></tr>)}
                </tbody>
              </table>
            </div>
