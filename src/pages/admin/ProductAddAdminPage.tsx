...
const ProductAddAdminPage: React.FC = () => {
            ...
            if (lastRound) {
                ...
                const mappedVGs: VariantGroupUI[] = (lastRound.variantGroups || []).map((vg: VariantGroup) => ({
                    ...
                    items: (vg.items || []).map((item: ProductItem) => ({
                        id: generateUniqueId(), name: item.name, price: item.price, limitQuantity: item.limitQuantity ?? '',
    ...
    // ✅ [오류 수정] waitlist, waitlistCount 속성 추가
    const getSalesRoundData = (status: SalesRoundStatus): Omit<SalesRound, 'roundId' | 'createdAt'> => {
      return {
          ...
          waitlist: [], // 초기값으로 빈 배열 추가
          waitlistCount: 0, // 초기값으로 0 추가
      };
    };

    // ✅ [개선] toast.promise를 사용하여 비동기 처리 개선
    const handleSubmit = async (isDraft: boolean = false) => {
        ...
        toast.promise(submissionPromise, {
            ...
            success: () => {
                ...
                if (mode === 'newProduct') {
                    return isDraft ? "상품이 임시저장되었습니다." : "신규 상품과 첫 회차가 성공적으로 등록되었습니다.";
                }
                return isDraft ? "새 회차가 임시저장되었습니다." : "새로운 판매 회차가 성공적으로 추가되었습니다.";
            },
            error: (err) => {
                setIsSubmitting(false);
                return `저장 실패: ${(err as Error).message}`;
            }
        });
    };
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(false); }}>
                ...
                <main className="main-content-grid-3-col-final">
                        ...
                        {variantGroups.map(vg => (
                            <div className="variant-group-card" key={vg.id}>
                                <div className="variant-group-header">
                                    ...
                                    {productType === 'group' && variantGroups.length > 1 && <button type="button" onClick={()=>removeVariantGroup(vg.id)} className="remove-variant-group-btn"><Trash2 size={14}/></button>}
                                </div>
                                {vg.items.map(item => (
                    ...
                    <div className="form-section sticky-section">
                        <div className="form-section-title"><div className="title-text-group"><SlidersHorizontal size={20} className="icon-color-settings"/><h3>발행 및 기간 설정</h3></div></div><p className="section-subtitle">상품의 분류, 보관 타입과 판매 시점 및 기간을 설정합니다.</p>
