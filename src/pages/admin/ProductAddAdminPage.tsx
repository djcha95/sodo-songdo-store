...
import { Image as ImageIcon, Save, PlusCircle, X, Loader, Package, Box, SlidersHorizontal, Trash2, LayoutGrid, Info, UploadCloud, FileText } from 'lucide-react';
...
const formatDateToYYYYMMDD = (date: Date | null): string => { if (!date) return ''; return date.toISOString().split('T')[0]; };
...
const getSmartDeadline = (): Date => { const now = new Date(); const dayOfWeek = now.getDay(); const nextDay = new Date(now); nextDay.setHours(13, 0, 0, 0); if (dayOfWeek >= 1 && dayOfWeek <= 4) { nextDay.setDate(now.getDate() + 1); } else { const daysUntilMonday = dayOfWeek === 5 ? 3 : (dayOfWeek === 6 ? 2 : 1); nextDay.setDate(now.getDate() + daysUntilMonday); } return nextDay; };
...
const ProductAddAdminPage: React.FC = () => {
    ...
    const removeImage = useCallback((index: number) => {
        ...
        const fileIndex = imageFiles.findIndex(f => URL.createObjectURL(f) === urlToRemove);
        setImagePreviews(p => p.filter((_, i) => i !== index));
        if (fileIndex > -1) setImageFiles(p => p.filter((_, i) => i !== fileIndex));
    ...
    const getSalesRoundData = (status: SalesRoundStatus): Omit<SalesRound, 'roundId' | 'createdAt'> => {
      return {
          ...
          variantGroups: variantGroups.map(vg => ({
              ...
              totalPhysicalStock: vg.totalPhysicalStock === '' ? null : Number(vg.totalPhysicalStock), stockUnitType: vg.stockUnitType,
    ...
    const handleSubmit = async (isDraft: boolean = false) => {
        ...
        try {
            if (mode === 'newProduct') {
                ...
                toast.success(isDraft ? "상품이 임시저장되었습니다." : "신규 상품과 첫 회차가 성공적으로 등록되었습니다.");
            } else if (mode === 'newRound' && existingProductId) {
                await addNewSalesRound(existingProductId, salesRoundToSave);
                toast.success(isDraft ? "새 회차가 임시저장되었습니다." : "새로운 판매 회차가 성공적으로 추가되었습니다.");
            }
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(false); }}>
                <header className="product-add-header">
                    ...
                    <div className="header-actions">
                        ...
                        <button type="submit" disabled={isSubmitting} className="save-button">{isSubmitting ? <Loader size={18} className="spin" /> : <Save size={18} />} {isSubmitting ? '저장 중...' : (mode === 'newProduct' ? '신규 상품 등록하기' : '새 회차 추가하기')}</button>
                    </div>
                </header>
                <main className="main-content-grid-2x2">
                    <div className="form-column-2x2-left">
                        {mode === 'newProduct' ? (
                            <div className="form-section">
                                ...
                                <p className="section-subtitle">고객에게 보여질 상품의 기본 정보와 첫 판매 회차 이름을 설정합니다.</p>
                             ...
                             <div className="form-section">
                                <div className="form-section-title"><div className="title-text-group"><Info size={20} className="icon-color-round"/><h3>새 회차 정보</h3></div></div>
                                <p className="section-subtitle">기존 상품에 대한 새로운 판매 회차의 이름과 옵션을 설정합니다.</p>
                                <div className="form-group"><label>회차명 *</label><input type="text" value={roundName} onChange={e=>setRoundName(e.target.value)} placeholder="예: 2차 판매" required/></div>
                            </div>
                        )}
                        ...
                        {mode === 'newProduct' && (
                          <div className="form-section">
                              <div className="form-section-title"><div className="title-text-group"><ImageIcon size={20} className="icon-color-image"/><h3>대표 이미지 *</h3></div></div>
                               <p className="section-subtitle">고객이 상품 목록에서 보게 될 대표 이미지입니다. (첫 이미지가 썸네일)</p>
                              <div className={`image-upload-area ${isDragging ? 'dragging' : ''}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={()=>!imageFiles.length && fileInputRef.current?.click()}><input type="file" ref={fileInputRef} onChange={handleFileChange} multiple accept="image/*" style={{display:'none'}}/>{imagePreviews.length > 0 ? (<div className="image-previews-grid-final">{imagePreviews.map((p,i) => (<div key={p+i} className="image-preview-item"><img src={p} alt=""/><button type="button" onClick={() => removeImage(i)} className="remove-image-btn"><X size={12}/></button></div>))}{imagePreviews.length < 10 && (<button type="button" onClick={()=>fileInputRef.current?.click()} className="add-image-btn-final"><PlusCircle size={24}/><span>추가</span></button>)}</div>) : (<div className="dropzone-prompt"><UploadCloud size={48}/><p>클릭 또는 드래그하여 업로드</p></div>)}</div>
                          </div>
                        )}
                    </div>
                    ...
                    <div className="form-column-2x2-right">
                        <div className="form-section">
                            <div className="form-section-title"><div className="title-text-group"><Box size={20} className="icon-color-option"/><h3>판매 옵션 설정 *</h3></div></div>
                             <p className="section-subtitle">실제 판매될 상품의 옵션과 가격, 재고 등을 설정합니다.</p>
                            {variantGroups.map(vg => (
                                <div className="variant-group-card" key={vg.id}>
                                    {productType === 'group' && (<div className="variant-group-header"><div className="form-group full-width"><label>하위 상품 그룹명 *</label><input type="text" value={vg.groupName} onChange={e=>handleVariantGroupChange(vg.id, 'groupName', e.target.value)} placeholder="예: GAP 인증 블루베리" required /></div>{variantGroups.length > 1 && <button type="button" onClick={()=>removeVariantGroup(vg.id)} className="remove-variant-group-btn"><X size={14}/></button>}</div>)}
                                    {vg.items.map(item => (
                                        <div className="option-item-card" key={item.id}>
                                            <div className="option-item-grid-final">
                                                <div className="form-group-grid item-name"><label>선택지 *</label><input type="text" value={item.name} onChange={e=>handleItemChange(vg.id, item.id, 'name', e.target.value)} placeholder="예: 1팩(500g)" required/></div>
