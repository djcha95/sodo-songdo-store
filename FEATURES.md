...
8.  **✨ 신뢰도 시스템 개편: '신뢰 등급'과 '활동 포인트' 이중 트랙 시스템**
    ...
    * **👑 1. 신뢰 등급 (Trust Tier): 사용자의 약속 이행에 대한 지표**
        ...
        * **등급 체계:**
            * **공구의 신:** 픽업률 98% 이상 & 누적 픽업 50회 이상
            * **공구왕:** 픽업률 95% 이상 & 누적 픽업 20회 이상
            * **공구요정:** 픽업률 90% 이상 & 누적 픽업 5회 이상
            * **공구새싹:** 그 외 모든 일반 사용자
            * **주의 요망:** 픽업률 70% 미만
            * **참여 제한:** 누적 노쇼 3회 이상
...
10. **✨ 지능형 알림톡 시스템 (Alimtalk Notifications)**
    ...
    * **1차 실행 (오전 9시 - 픽업 안내):**
        ...
        - 픽업 기간이 여러 날일 경우, 가장 빠른 마감일을 안내하여 혼동을 줄입니다.
    * **2차 실행 (오후 7시 - 선입금 안내):**
        - 매일 저녁 7시, `sendPrepaymentReminders` 함수가 실행됩니다.
        - **'오늘이 픽업 마감일'**인 상품을 아직 찾아가지 않은 고객에게만, 마감 1시간 전에 '선입금' 전환을 안내하는 최종 리마인더를 발송하여 포인트 차감 등의 불이익을 막을 수 있는 구제책을 제공합니다.
    * **핵심 파일:** `functions/src/index.ts` (sendPickupReminders, sendPrepaymentReminders), `NHN Cloud Console` (템플릿 관리)
