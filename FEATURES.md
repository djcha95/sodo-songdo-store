...
5.  **대기(Waitlist) 기능**
    ...
    * **핵심 파일:** `ProductDetailPage.tsx`, `OrderHistoryPage.tsx`, `functions/src/callable/waitlist.ts`
...
8.  **✨ 신뢰도 시스템 개편: '신뢰 등급'과 '활동 포인트' 이중 트랙 시스템**
    ...
    * **💰 2. 활동 포인트 (Activity Points): 서비스 활동에 대한 보상**
        ...
        * **✅ [수정] 포인트 사용처는 현재 논의 중이며, 추후 상품 할인, 특별 기능 이용 등에 사용될 예정입니다.**
...
11. **✨ 지능형 알림톡 시스템 (Alimtalk Notifications)**
...
-   **페이지 입장/결제 직전** → 서버에서 최종 실재고 검증
-   수량 자동 보정 또는 품절 처리
-   변경 내역은 프론트에서 토스트 알림으로 안내
...
| 구분 | 획득 | 차감 | 소멸 | 비고 |
|---|---|---|---|---|
| Activity Points | 주문, 출석 | 취소 | 매일 0시 일부 소멸 | 단기 성과 |
| Trust Tier | 누적 활동 | - | 장기 유지 | 회원 등급 |
...
| 시간 | 대상 | 메시지 |
|---|---|---|
| 매일 09:00 | 당일 픽업자 | 픽업 알림 |
| 매일 19:00 | 선입금 필요자 | 입금 요청 |
| 마감 임박 | 모든 예약자 | 재고 부족/변경 안내 |